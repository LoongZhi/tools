...
#define playViewHeight  ([[UIScreen mainScreen] bounds].size.height - (lzkTopSafeHeight + lzBottomSafeHeight))
...
{
    _player = [[VLCPlayer alloc] initWithView:self.vlcPlayerView.playView andMediaPath:m_playURL];
    _player.player.delegate = self;
    NSMutableDictionary *mediaDictionary = [[NSMutableDictionary alloc] init];
    NSString *lastPath = [[[NSURL fileURLWithPath:m_playURL] absoluteString] pathExtension];
...
- (void)addVLCPalyerView
{
    ...
    self.vlcPlayerView = [[VLCPlayerView alloc] initWithFrame:CGRectMake(0, lzkTopSafeHeight, [UIScreen mainScreen].bounds.size.width, playViewHeight)];
    // 设置视频名称
...
- (void)changeProgress:(UISlider *)sliderView
{
    self.currentProgress = [_player.player position];
      ...
      self.videoCurrentTime = abs([[_player.player time] intValue] / 1000);
      // 根据剩余时间和已经播放的计算总秒数（这里不够严格，还得加上秒数）
      self.videoAllTime = abs([[_player.player remainingTime] intValue]/1000) + self.videoCurrentTime;
